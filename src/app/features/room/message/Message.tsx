...
export const getMessageMxcUri = (mEvent: MatrixEvent): string | undefined => {
  const content = mEvent.getContent<Record<string, unknown>>();
  const file =
    content.file && typeof content.file === 'object' && !Array.isArray(content.file)
      ? (content.file as Record<string, unknown>)
      : undefined;
  const uri = file?.url ?? content.url;
  return typeof uri === 'string' && uri.startsWith('mxc://') ? uri : undefined;
};

export const MessageCopyMxcItem = as<
  'button',
  {
    mxcUri: string;
    onClose?: () => void;
  }
>(({ mxcUri, onClose, ...props }, ref) => {
  const handleCopy = () => {
    copyToClipboard(mxcUri);
    onClose?.();
  };
...
export const Message = as<'div', MessageProps>(
  (
    ...
    const messageMxcUri = getMessageMxcUri(mEvent);
                    ...
                    >
                      <Menu>
                        ...
                        <Box direction="Column" gap="100" className={css.MessageMenuGroup}>
                          ...
                          {messageMxcUri && (
                            <MessageCopyMxcItem mxcUri={messageMxcUri} onClose={closeMenu} />
                          )}
                          {canPinEvent && (
