...
export enum Command {
  ...
  Delete = 'delete',
  Acl = 'acl',
  // Ported from cinny-old's slash table (src/lib/slash.ts).
  Topic = 'topic',
  RoomNick = 'nick',
  WhoIs = 'whois',
  Op = 'op',
...
export const useCommands = (mx: MatrixClient, room: Room): CommandRecord => {
  ...
  const commands: CommandRecord = useMemo(
    () => ({
      ...
      [Command.Plain]: {
        name: Command.Plain,
        description: 'Send message as plain text without markdown. Example: /plain message',
        exe: async () => undefined,
      },
      [Command.TableFlip]: {
      ...
      // --- cinny-old ports (src/lib/slash.ts) ---
      [Command.Topic]: {
        name: Command.Topic,
        description: 'Change room topic. Example: /topic new topic',
        exe: async (payload) => {
          await mx.setRoomTopic(room.roomId, payload.trim() || undefined);
        },
      },
      [Command.RoomNick]: {
        name: Command.RoomNick,
        description: 'Change display name. Example: /nick New Name',
        exe: async (payload) => {
          const name = payload.trim().slice(0, 256);
          if (!name) return;
      ...
      [Command.WhoIs]: {
        name: Command.WhoIs,
        description: 'Open profile of user. Example: /whois @user:server',
        exe: async (payload) => {
          const rawIds = splitWithSpace(payload);
          const userId = rawIds.find((id) => isUserId(id));
          if (!userId) return;
          navigateRoom(room.roomId);
          window.dispatchEvent(
            new CustomEvent('cinny:open-profile', { detail: { userId, roomId: room.roomId } })
          );
        },
      },
      [Command.Op]: {
        name: Command.Op,
        description: 'Give moderator power. Example: /op @user:server [50]',
        exe: async (payload) => {
          const parts = splitWithSpace(payload);
          const target = parts.find((id) => isUserId(id));
          if (!target) return;
          const levelRaw = parts.find((p) => p !== target);
          const level = levelRaw === undefined ? 50 : Number(levelRaw);
          if (!Number.isSafeInteger(level) || !/^-?\d+$/.test(levelRaw ?? '50')) return;
          await mx.setPowerLevel(room.roomId, target, level);
        },
      },
      [Command.DeOp]: {
        name: Command.DeOp,
        description: 'Remove power-level override. Example: /deop @user:server',
        exe: async (payload) => {
          const rawIds = splitWithSpace(payload);
          const target = rawIds.find((id) => isUserId(id));
          if (!target) return;
          await mx.setPowerLevel(room.roomId, target, undefined);
        },
      },
      [Command.DiscardSession]: {
        name: Command.DiscardSession,
        description: 'Discard outbound encryption session and start a new one.',
        exe: async () => {
      ...
      [Command.UpgradeRoom]: {
        ...
        description: 'Upgrade room to a new version. Example: /upgraderoom 11',
        exe: async (payload) => {
          const version = payload.trim();
          if (!version || version.length > 32) return;
          // eslint-disable-next-line no-alert
          if (!window.confirm(`Upgrade ${room.roomId} to room version ${version}?`)) return;
          const result = await mx.upgradeRoom(room.roomId, version);
          navigateRoom(result.replacement_room);
        },
      },
    }),
