...
const useNotificationTimeline = (
  ...
  decryptEncryptedRooms?: boolean
): [NotificationTimeline, LoadTimeline, SilentReloadTimeline] => {
  const mx = useMatrixClient();
  const allRooms = useAtomValue(allRoomsAtom);
  const allJoinedRooms = useMemo(() => new Set(allRooms), [allRooms]);
  const roomsById = useMemo(() => new Map(mx.getRooms().map((room) => [room.roomId, room])), [mx]);

  const [notificationTimeline, setNotificationTimeline] = useState<NotificationTimeline>({
    groups: [],
  });

  const resolveEncryptedNotificationEvents = useCallback(
    async (notifications: INotification[]): Promise<INotification[]> => {
      if (!decryptEncryptedRooms || !mx.getCrypto()) return notifications;
      const crypto = mx.getCrypto() as CryptoBackend;
      const resolved = await Promise.all(
        notifications.map(async (notification) => {
          if (
            notification.event.type !== EventType.RoomMessageEncrypted ||
            typeof notification.event.event_id !== 'string'
          ) {
            return notification;
          }
          const room = roomsById.get(notification.room_id) ?? mx.getRoom(notification.room_id);
          const localEvent =
            room?.findEventById(notification.event.event_id) ??
            (notification.event.event_id
              ? room
                  ?.getTimelineForEvent(notification.event.event_id)
                  ?.getEvents()
                  .find((evt) => evt.getId() === notification.event.event_id)
              : undefined);
          if (localEvent && !localEvent.isDecryptionFailure()) {
            return notification;
          }
          ...
          const [decryptErr] = await to(mEvent.attemptDecryption(crypto));
          if (
            decryptErr ||
    ...
    [decryptEncryptedRooms, mx, roomsById]
  );
  ...
  const loadTimeline: LoadTimeline = useCallback(
    async (from) => {
      ...
      const resolvedNotifications = await resolveEncryptedNotificationEvents(data.notifications);
      const groups = groupNotifications(resolvedNotifications, allJoinedRooms);
    ...
    [
      ...
      fetchNotifications,
      allJoinedRooms,
      resolveEncryptedNotificationEvents,
    ]
  );
...
export function Notifications({ mentionsOnly = false }: { mentionsOnly?: boolean }) {
  const mx = useMatrixClient();
  ...
  const onlyHighlight = mentionsOnly || notificationsSearchParams.only === 'highlight';
  const decryptMentionedEvents = mentionsOnly;
    ...
    <Page>
      <PageHeader balance>
        <Box grow="Yes" gap="200">
          ...
          <Box alignItems="Center" gap="200">
            {screenSize !== ScreenSize.Mobile && (
              <Icon size="400" src={mentionsOnly ? Icons.Mention : Icons.Message} />
            )}
            <Text size="H3" truncate>
              {mentionsOnly ? 'Recent Mentions' : 'Notification Messages'}
            </Text>
          </Box>
          <Box grow="Yes" basis="No" />
        </Box>
            ...
            <PageContentCenter>
              <Box direction="Column" gap="200">
                {!mentionsOnly ? (
                  ...
                  <span ref={scrollTopAnchorRef} data-spacing-node />
                )}
                <ScrollTopContainer
                  scrollRef={scrollRef}
                    ...
                    return (
                      ...
                      >
                        <RoomNotificationsGroupComp
                          ...
                          mediaAutoLoad={mediaAutoLoad}
                          urlPreview={
                            urlPreview && (!groupRoom.hasEncryptionStateEvent() || encUrlPreview)
                          }
