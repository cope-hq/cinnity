...
export function Space() {
  ...
  const favouriteCategoryId = makeNavCategoryId(space.roomId, 'favourites');
  const spaceFavouriteRooms = useMemo(
    () =>
      allRooms
        .filter((roomId) => {
          const room = getRoom(roomId);
          return (
            favouriteRooms.has(roomId) &&
            room !== undefined &&
            !room.isSpaceRoom() &&
            getAllParents(roomToParents, roomId).has(space.roomId)
          );
        })
        .sort(factoryRoomIdByAtoZ(mx)),
    [allRooms, favouriteRooms, getRoom, mx, roomToParents, space.roomId]
  );
  const shownFavouriteRooms = closedCategories.has(favouriteCategoryId)
    ? spaceFavouriteRooms.filter(
        (roomId) =>
  ...
  const hierarchy = useSpaceJoinedHierarchy(
    ...
    useCallback(
      (parentId, roomId) => {
        if (favouriteRooms.has(roomId)) return true;
      ...
      [space.roomId, favouriteRooms, closedCategories, roomToUnread, selectedRoomId, callEmbed]
    ),
    useCallback(
    ...
    <PageNav>
      ...
      <PageNavContent scrollRef={scrollRef}>
        <Box direction="Column" gap="300">
          ...
          {spaceFavouriteRooms.length > 0 && (
            <NavCategory>
              <NavCategoryHeader>
                <RoomNavCategoryButton
                  data-category-id={favouriteCategoryId}
                  onClick={handleCategoryClick}
                  closed={closedCategories.has(favouriteCategoryId)}
                >
                  Favourites
                </RoomNavCategoryButton>
              </NavCategoryHeader>
              {shownFavouriteRooms.map((roomId) => {
